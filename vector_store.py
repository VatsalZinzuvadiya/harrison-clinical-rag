"""
Embedding & Vector Store Management Module

Supports high-speed Jina AI Cloud Embeddings API (jina-embeddings-v3) with
seamless fallback to local Hugging Face SentenceTransformers (BGE family).
...
class LocalVectorStore:
    """
    Vector Store wrapper utilizing ChromaDB with Jina AI API & local BGE model fallback.
    Supports batched insertion, metadata filtering, and hardware acceleration (CUDA/CPU).
    """
    ...
    def __init__(self, model_name: str = config.EMBEDDING_MODEL_NAME,
        ...
        # Detect GPU / CPU hardware execution device for local embedder
    ...
    def is_indexed(self) -> bool:
        """Returns True if collection already contains vector entries."""
    ...
    def _embed_texts_jina(self, texts: List[str], is_query: bool = False) -> List[List[float]]:
        """
        Embeds a list of text strings using Jina AI Cloud API with socket pooling.
        """
        ...
        # Fallback to local embedder if Jina API fails
    ...
    def _embed_texts_local(self, texts: List[str], is_query: bool = False) -> List[List[float]]:
        """
        Embeds a list of text strings using local SentenceTransformer model.
        """
        if self.embedder is None:
            if SentenceTransformer is None:
                raise ImportError("sentence-transformers is required for local embedding fallback.")
            self.embedder = SentenceTransformer(self.model_name, device=self.device)
    ...
    def _embed_texts(self, texts: List[str], is_query: bool = False) -> List[List[float]]:
        """
        Embeds texts using Jina AI API if enabled, otherwise local model.
        """
        if self.use_jina:
    ...
    def build_index(self, chunks: List[Dict[str, Any]], force_rebuild: bool = False) -> None:
        """
        Embeds text chunks in batches and stores them persistently in Chroma.
        """
    ...
    def dense_search(self, query: str, top_k: int = config.DENSE_TOP_K) -> List[Dict[str, Any]]:
        """
        Performs dense cosine-similarity search for a query string.
        """
...
if __name__ == "__main__":
    ...
    print(f"Vector Store Status: Indexed={store.is_indexed()}, Total Count={store.get_count()}")
