"""
Local Clinical LLM Generator & Prompt Engineering Module

Connects to a free local LLM runtime via Ollama API (e.g., Llama 3.2, Mistral).
Enforces zero-hallucination clinical system prompt rules: page citations, refusal when
information is absent, structured differential presentation, and medical safety disclaimers.
"""
...
class LocalClinicalGenerator:
    """
    Generator communicating with local Ollama runtime over HTTP.
    Includes automatic connection checks and fallback summary handling.
    """
    ...
    def check_llm_status(self) -> Dict[str, Any]:
        """
        Checks availability of Groq API and local Ollama server.
        """
    ...
    def _build_user_prompt(self, query: str, context_chunks: List[Dict[str, Any]]) -> str:
        """
        Formats retrieved context chunks into a structured prompt context block.
        """
        context_str_parts = []
    ...
    def _generate_with_groq(self, query: str, context_chunks: List[Dict[str, Any]]) -> Optional[str]:
        """
        Sends formatted context and clinical query to Groq Cloud API.
        Includes automatic model failover between gpt-oss-120b, qwen3.8-27b, and gpt-oss-20b.
        """
    ...
    def generate(self, query: str, context_chunks: List[Dict[str, Any]]) -> str:
        """
        Sends formatted context and clinical query to Groq API (or Ollama fallback).
        """
        ...
        # 1. Try Groq Cloud API first (ultra-fast Llama 3.3 70B)
        ...
        # 2. Check local Ollama connection fallback
        ...
        try:
            ...
            print(f"[LocalClinicalGenerator] Ollama API returned status {response.status_code}. Using fallback.")
            return self._fallback_grounded_summary(query, context_chunks, error_msg=f"HTTP {response.status_code}")

        except Exception as e:
            print(f"[LocalClinicalGenerator] Failed to reach Ollama at {self.ollama_url}: {e}")
    ...
    def _fallback_grounded_summary(self, query: str, context_chunks: List[Dict[str, Any]],
                                   ollama_missing: bool = False,
                                   error_msg: str = "") -> str:
        """
        Fallback deterministic response generator when Ollama is offline.
        Extracts key sentences with direct page citations.
        """
        notice = ""
        if ollama_missing:
            notice = (
...
if __name__ == "__main__":
    gen = LocalClinicalGenerator()
    status = gen.check_ollama_status()
